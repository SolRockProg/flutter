...
abstract class FlutterCommand extends Command<void> {
  ...
  void addDevToolsOptions({required bool verboseHelp}) {
    argParser.addFlag(kEnableDevTools,
        hide: !verboseHelp,
        defaultsTo: true,
        help:
  ...
  bool get _hostVmServicePortProvided =>
      argResults?.wasParsed('observatory-port') == true ||
  ...
  int get ddsPort {
    if (argResults?.wasParsed('dds-port') != true &&
  ...
  void usesAnalyzeSizeFlag() {
    ...
    argParser.addOption(FlutterOptions.kCodeSizeDirectory,
        help:
  ...
  Future<BuildInfo> getBuildInfo(
      {BuildMode? forcedBuildMode, File? forcedTargetFile}) async {
    ...
    final File packagesFile = globals.fs.file(packagesPath ??
    ...
    if (argParser.options.containsKey(FlutterOptions.kAnalyzeSize) &&
      ...
      if (argParser.options.containsKey(FlutterOptions.kCodeSizeDirectory) &&
        ...
        directory =
            globals.fs.directory(stringArg(FlutterOptions.kCodeSizeDirectory));
      }
    ...
    if (argParser.options.containsKey(FlutterOptions.kNullSafety)) {
      ...
      final bool wasNullSafetyFlagParsed =
          argResults?.wasParsed(FlutterOptions.kNullSafety) == true;
      if (!wasNullSafetyFlagParsed &&
          (argParser.options.containsKey('target') ||
              forcedTargetFile != null)) {
        final File entrypointFile =
            forcedTargetFile ?? globals.fs.file(targetFile);
    ...
    if (buildMode != BuildMode.release && codeSizeDirectory != null) {
      throwToolExit(
          '"--${FlutterOptions.kAnalyzeSize}" can only be used on release builds.');
    }
    ...
    return BuildInfo(
      ...
      extraFrontEndOptions:
          extraFrontEndOptions.isNotEmpty ? extraFrontEndOptions : null,
      extraGenSnapshotOptions:
          extraGenSnapshotOptions.isNotEmpty ? extraGenSnapshotOptions : null,
  ...
  static List<String> updateDartDefines(
    ...
    final Iterable<String>? webRendererDefine =
        _webRendererDartDefines[webRenderer];
  ...
  List<String> stringsArg(String name) =>
      argResults?[name] as List<String>? ?? <String>[];
}
